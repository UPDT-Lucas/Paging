...
export class MRU implements IMMU {
  ...
  swapAndCreatePage(): Page {
    ...
    const newPage: Page = new Page(this.pageConsecutive++, true, true, segmentReuse, 4096);
  ...
  cUsePointer(pointerId: number): void{
    ...
    this.loadedPages.unshift(...mruPages);
  }
  ...
  replacePageUse(pages: Page[]): number | undefined{
    ...
    const replacePages = this.loadedPages.filter((page) => !this.loadedPages.includes(page));
    const pageToReplace = replacePages[0];
  ...
  swapingPages(): number {
    const pageId: number | undefined = this.recentlyUsedPages.pop();
    for (const [key, values] of this.pointerPageMap) {
      for (const value of values) {
        if (value.getId() === pageId) {
          //this.fifoVirtualPages.push(pageId);
